    ...
    usize: From<T> + From<<T as Sub>::Output>,
{
    ...
    pub fn min_coins(&self, amount: T) -> Option<T> {
        ...
        let mut change: Vec<Option<T>> = vec![None; 1 + index];
        change[0] = Some(zero);
        ...
        while i <= amount {
            for coin in 0..self.coins.len() {
                    ...
                    && value <= i
                {
                    ...
                    if let Some(cur_count) = change[first] {
                        if let Some(old_value) = change[second] {
                            let ov: T = old_value;
                            change[second] = Some(ov.min(cur_count + one))
                        } else {
                            change[second] = Some(cur_count + one);
                        }
                    }
                }
            }
...
mod test {
    ...
    #[test]
    fn failed_test() {
        let cc = CoinChange::new(&[186, 419, 83, 408]);
        assert_eq!(Some(20), cc.min_coins(6249));
    }

    #[test]
    fn ex1() {
        let cc = CoinChange::new(&[1, 2, 5]);
    ...
    #[test]
    fn ex2() {
        let cc = CoinChange::new(&[2]);
        assert_eq!(None, cc.min_coins(3));
    }

    #[test]
    fn ex3() {
        let cc = CoinChange::new(&[1]);
